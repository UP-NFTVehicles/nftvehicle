...
async function callingSetHelper(req){    
    ...
    const typeHelper = req.body.typeHelper;
    ...
    try {
        ...
        await new Promise(async (resolve,reject) => {
            try {
                ...
                const contractAnswer = await userContract.methods.setHelper(tokenId,helper,about,typeHelper).send({from: legalOwner, gas:gas })
...
initializer.setHelper = async function (req, res){
    const gas = req.body.gas; 
    const contractAdd = req.body.contractAdd; 
    const legalOwner = req.body.legalOwner;
    const tokenId = req.body.tokenId;
    ...
    var obj={body:
		...
		if(errNum){  //				
			...
			try {
				const response = await callingSetHelper(obj).then((resul)=>{
