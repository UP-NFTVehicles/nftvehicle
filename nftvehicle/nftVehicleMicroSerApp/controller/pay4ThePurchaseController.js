...
async function callingPay4ThePurchase(req){    
    ...
    try {
        ...
        await new Promise(async (resolve,reject) => {
            try {
                ...
                const contractAnswer = await userContract.methods.payForThePurchase(legalOwner,tokenId).send({from: possibleLegalOwner, gas:gas, value:priceProposal})
...
initializer.pay4ThePurchase = async function (req, res){
    const gas = req.body.gas; 
    const contractAdd = req.body.contractAdd; 
    const legalOwner = req.body.legalOwner;
    const priceProposal = req.body.priceProposal;
    ...
    var obj={body:
		...
		if(errNum){  //				
			...
			try {
				const response = await callingPay4ThePurchase(obj).then((resul)=>{
